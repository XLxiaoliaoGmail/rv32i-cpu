...
控制流程：
   1. 复位或空闲，将self-ready置1（表示可接收数据），self-valid置0（表示无有效数据）
...
## 缓存
  实现指令与数据缓存机制，使用AXI协议与存储器通信。
  
  指令缓存仅支持读取，数据缓存可读取与写入。
  
  数据缓存模块运行描述如下图所示，指令缓存与此类似。

控制流程：
  1. 空闲，等待有效的缓存请求
  3. 若有请求，检查缓存是否命中
  5. 若命中，则立即读/写
  4. 若未命中且欲替换的缓存行为脏，则写回旧数据，从内存重填新数据
  4. 若无需写回，直接从内存重填新数据
  5. 若为写，则写入缓存
